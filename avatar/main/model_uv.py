...
class Model(nn.Module):
    ...
    def forward(self, data, cur_itr, mode):
        ...
        for i in range(batch_size):
            ...
            key_list = ['mean_3d', 'scale', 'rotation', 'rgb','opacity']
            ...
            for key in ['img', 'mask','mean_2d', 'is_vis', 'radius']:
        ...
        # breakpoint()
        human_renders_refined = {k: (torch.stack(v) if k!='mean_2d' else v) for k,v in human_renders_refined.items()}
        ...
        if mode == 'train':
            ...
            stats = {'mean_2d': scene_renders['mean_2d'], 'is_vis': scene_renders['is_vis'].detach(), 'radius': scene_renders['radius'].detach(),
                     'mean_2d_human':human_renders_refined['mean_2d'], 'is_vis_human':human_renders_refined['is_vis'].detach(), 'radius_human':human_renders_refined['radius'].detach()}
            ...
            weight = torch.ones((1,self.human_gaussian.pos_enc_uv.shape[0],1)).float().cuda() * 10
            weight[:,self.human_gaussian.is_rhand_uv,:] = 1000
            weight[:,self.human_gaussian.is_lhand_uv,:] = 1000
            weight[:,self.human_gaussian.is_face_uv,:] = 1
            weight[:,self.human_gaussian.is_face_expr_uv,:] = 10
            loss['gaussian_mean_reg'] = (human_offsets['mean_offset'] ** 2 + human_offsets['mean_offset_offset'] ** 2) * weight
            # loss['gaussian_mean_hand_reg'] = self.hand_mean_reg(mesh_neutral_pose, human_offsets['mean_offset'], self.human_gaussian.is_lhand_uv, self.human_gaussian.is_rhand_uv) + self.hand_mean_reg(mesh_neutral_pose, human_offsets['mean_offset_offset'], self.human_gaussian.is_lhand_uv, self.human_gaussian.is_rhand_uv)
            weight = torch.ones((1,self.human_gaussian.pos_enc_uv.shape[0],1)).float().cuda() 
            weight[:,self.human_gaussian.is_rhand_uv,:] = 1000 
            weight[:,self.human_gaussian.is_lhand_uv,:] = 1000
            weight[:,self.human_gaussian.is_face_expr_uv,:] = 10
            weight[:,self.human_gaussian.is_cavity_uv,:] = 0
            if cfg.is_warmup:
            ...
            # loss['hand_rgb_reg'] = (self.hand_rgb_reg(human_assets['rgb'], self.human_gaussian.is_rhand_uv, self.human_gaussian.is_lhand_uv) + self.hand_rgb_reg(human_assets_refined['rgb'], self.human_gaussian.is_rhand_uv, self.human_gaussian.is_lhand_uv)) * 0.01
            ...
            return human_assets_refined, stats, loss
        else:
            out = {}
    ...
    def adjust_human_gaussians(self, human_gs_out, track, human_gaussian, cur_itr, optimizer):
        batch_size = track['mean_2d_grad_human'].shape[0]
        for i in range(batch_size):
            human_gaussian.max_radii2D[track['is_vis_human'][i]] = torch.maximum(human_gaussian.max_radii2D[track['is_vis_human'][i]], track['radius_human'][i][track['is_vis_human'][i]])
            # breakpoint()
            human_gaussian.add_densification_stats(track['mean_2d_grad_human'][i], track['is_vis_human'][i])
        
        if (cur_itr > cfg.densify_start_itr) and (cur_itr % cfg.densify_interval == 0):
            size_threshold = 20 if cur_itr > cfg.opacity_reset_interval else None
            human_gaussian.densify_and_prune(human_gs_out, cfg.densify_grad_thr, cfg.prune_min_opacity, cfg.densify_extent, size_threshold, optimizer, max_n_gs=cfg.max_n_gaussians)
...
def get_model(scene, cam_dist, scene_point_num, smplx_params):
    ...
    human_gaussian = HumanUVGaussian()
    
    with torch.no_grad():
        human_gaussian.init()
    human_gaussian.pretrain_init()
